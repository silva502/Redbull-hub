...
local C_BG      = Color3.fromRGB(10, 2, 2)
local C_PANEL   = Color3.fromRGB(25, 8, 8)
local C_ROW     = Color3.fromRGB(35, 12, 12)
local C_ROW_HOV = Color3.fromRGB(55, 20, 20)
local C_BORDER  = Color3.fromRGB(80, 20, 20)
local C_BORDER2 = Color3.fromRGB(180, 40, 40)
local C_HEADER  = Color3.fromRGB(15, 4, 4)
local C_ACCENT  = Color3.fromRGB(255, 200, 200)
local C_ACCENT2 = Color3.fromRGB(255, 80, 80)
local C_DIM     = Color3.fromRGB(150, 80, 80)
local C_WHITE   = Color3.fromRGB(255, 255, 255)
local C_ON_BG   = Color3.fromRGB(200, 40, 40)
local C_OFF_BG  = Color3.fromRGB(30, 20, 20)
local C_KEY_BG  = Color3.fromRGB(12, 2, 2)
...
local function autoSaveConfig()
        ...
        task.delay(0.5, function()
                ...
                pcall(function() writefile("RedbullHubConfig.json", HttpService:JSONEncode(cfg)) end)
                saveDebounce = false
        end)
end
...
-- ================= PANNEAU MOBILE (red theme) =================
local function createMobilePanel()
        local panel = Instance.new("ScreenGui")
        panel.Name = "RedbullHubButtons"
        ...
        local CORNER = 16
        local GAP = 12
        local COL1_X = -(BTN_W + GAP + BTN_W + 20)
        local COL2_X = -(BTN_W + 20)
        local BASE_Y = 0.35
        ...
        local function makeSeparateBtn(name, text, defaultPos, saveKey, callback, isToggle)
                ...
                btn.MouseButton1Down:Connect(function()
                        if not isToggle then container.BackgroundColor3 = Color3.fromRGB(200,50,50); btn.TextColor3 = Color3.fromRGB(0,0,0) end
                end)
                btn.MouseButton1Up:Connect(function()
                        if not isToggle then container.BackgroundColor3 = Color3.fromRGB(40,10,10); btn.TextColor3 = Color3.fromRGB(255,255,255) end
                end)
        ...
        local dropBRSetActive = makeSeparateBtn("BtnDropBR", "DROP\nBR",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[1]), "RedbullBtn_dropbr.txt",
        ...
        local autoLeftSetActive = makeSeparateBtn("BtnAutoLeft", "AUTO\nLEFT",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[1]), "RedbullBtn_autoleft.txt",
                function(setActive)
        ...
        local autoBatSetActive = makeSeparateBtn("BtnAutoBat", "BAT\nAIMBOT",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[2]), "RedbullBtn_autobat.txt",
                function(setActive)
        ...
        local autoRightSetActive = makeSeparateBtn("BtnAutoRight", "AUTO\nRIGHT",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[2]), "RedbullBtn_autoright.txt",
                function(setActive)
        ...
        local tpDownSetActive = makeSeparateBtn("BtnTpDown", "TP\nDOWN",
                UDim2.new(1, COL1_X, BASE_Y, ROW_Y[3]), "RedbullBtn_tpdown.txt",
                function(setActive, currentActive)
                        tpToGround()
        ...
        local carrySpeedSetActive = makeSeparateBtn("BtnCarrySpd", "CARRY\nSPD",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[3]), "RedbullBtn_carryspd.txt",
                function(setActive)
                        toggleSpeedType()
        ...
        local laggerSetActive = makeSeparateBtn("BtnLaggerMode", "LAGGER\nMODE",
                UDim2.new(1, COL2_X, BASE_Y, ROW_Y[4]), "RedbullBtn_laggermode.txt",
                function(setActive)
                        toggleLagger()
...
for _, name in pairs({"JispiHubGUI","RedbullHubGUI","VexulHubGUI"}) do
...
gui.Name="RedbullHubGUI"; gui.ResetOnSpawn=false; gui.DisplayOrder=10
gui.IgnoreGuiInset=true; gui.Parent=LP:WaitForChild("PlayerGui")

-- Steal Bar Setup (red theme)
...
progressRadLbl.TextColor3 = C_WHITE
...
barTrack.BackgroundColor3 = Color3.fromRGB(40, 20, 20)
...
barFill.BackgroundColor3 = C_ACCENT2
...
local function saveStealBarPosition()
    ...
    local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
    pcall(function() writefile("RedbullStealBarPos.txt", str) end)
end

local function loadStealBarPosition()
    local savedPos = nil
    pcall(function() savedPos = readfile("RedbullStealBarPos.txt") end)
    if savedPos and savedPos ~= "" then
        local parts = {}
        for part in string.gmatch(savedPos, "[^,]+") do table.insert(parts, part) end
        if #parts >= 4 then
...
local function saveMainPosition()
        ...
        local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
        pcall(function() writefile("RedbullHubGUIPos.txt", str) end)
end
...
local function saveMiniPosition()
        if miniBtn then
                ...
                local str = string.format("%.3f,%.1f,%.3f,%.1f", pos.X.Scale, pos.X.Offset, pos.Y.Scale, pos.Y.Offset)
                pcall(function() writefile("RedbullMiniPos.txt", str) end)
        end
end

local function loadMainPosition()
        local savedPos = nil
        pcall(function() savedPos = readfile("RedbullHubGUIPos.txt") end)
        if savedPos and savedPos ~= "" then
                local parts = {}
                for part in string.gmatch(savedPos, "[^,]+") do table.insert(parts, part) end
                if #parts >= 4 then
...
local function loadMiniPosition()
        local savedPos = nil
        pcall(function() savedPos = readfile("RedbullMiniPos.txt") end)
...
titleLbl.BackgroundTransparency=1; titleLbl.Text="REDBULL HUB"
titleLbl.TextColor3=C_ACCENT2; titleLbl.Font=Enum.Font.GothamBlack; titleLbl.TextSize=16
...
closeBtn.MouseEnter:Connect(function()
        TweenService:Create(closeBtn,TweenInfo.new(0.12),{BackgroundColor3=C_ACCENT2,TextColor3=C_BG}):Play()
        TweenService:Create(closeBtnStroke,TweenInfo.new(0.12),{Color=C_WHITE}):Play()
end)
closeBtn.MouseLeave:Connect(function()
        TweenService:Create(closeBtn,TweenInfo.new(0.12),{BackgroundColor3=C_HEADER,TextColor3=C_ACCENT}):Play()
        TweenService:Create(closeBtnStroke,TweenInfo.new(0.12),{Color=C_BORDER}):Play()
end)
...
miniBtn.Name = "RedbullMiniButton"
miniBtn.Size = UDim2.new(0,150,0,30)
miniBtn.Position = UDim2.new(0,8,0,66)
miniBtn.BackgroundColor3 = C_BG
miniBtn.BackgroundTransparency = 0
miniBtn.BorderSizePixel = 0
miniBtn.Text = "REDBULL HUB"
miniBtn.TextColor3 = C_ACCENT2
...
miniBtn.MouseEnter:Connect(function()
        TweenService:Create(miniBtn, TweenInfo.new(0.1), {BackgroundColor3 = C_ROW}):Play()
end)
miniBtn.MouseLeave:Connect(function()
        TweenService:Create(miniBtn, TweenInfo.new(0.1), {BackgroundColor3 = C_BG}):Play()
end)
...
UIS.InputBegan:Connect(function(input, gameProcessed)
        ...
        if input.KeyCode == toggleKey then
                ...
                closeBtn.Visible = main.Visible
                miniBtn.Visible = not main.Visible
                
                UserInputService.MouseIconEnabled = main.Visible
        end
end)
-- ==========================================
...
local function makeStatusRow(label, valTxt)
        ...
        val.BackgroundTransparency=1; lbl.Text=label; lbl.TextColor3=C_ACCENT; val.Font=Enum.Font.GothamBlack
...
local function makeToggleRow(label, defaultKey, defaultOn, onToggle, onKeyChanged, keybindStateKey)
        local row=Instance.new("Frame",scroll); row.Size=UDim2.new(1,0,0,26); row.BackgroundColor3=C_ROW; row.LayoutOrder=LO()
        Instance.new("UICorner",row).CornerRadius=UDim.new(0,8)
        ...
        if keybindStateKey then
                ...
                newKeyBtn.BackgroundColor3 = C_KEY_BG
                newKeyBtn.BorderSizePixel = 0
                newKeyBtn.Text = keyDisplayName
                newKeyBtn.TextColor3 = C_ACCENT2
                ...
                local function nkStop(key)
                        ...
                        TweenService:Create(nks, TweenInfo.new(0.12), {Color=C_BORDER2}):Play()
                        newKeyBtn.TextColor3 = C_ACCENT2
                        if key then
        ...
        local pillBg=Instance.new("Frame",row); pillBg.Size=UDim2.new(0,34,0,17); pillBg.Position=UDim2.new(1,-40,0.5,-8)
        ...
        local dot=Instance.new("Frame",pillBg); dot.Size=UDim2.new(0,11,0,11); dot.Position=defaultOn and UDim2.new(1,-14,0.5,-6) or UDim2.new(0,3,0.5,-6)
...
laggerBox = makeInputRow("Lagger Speed", State.laggerSpeed, function(v) local n=tonumber(v); if n and n>0 and n<=500 then State.laggerSpeed=n end end)
...
autoSaveLbl.TextColor3 = C_ACCENT
...
footerLbl.Text="redbull hub  ·  v1.0"; footerLbl.TextColor3=C_DIM
...
local function loadConfig()
        local hasFile=false; pcall(function() hasFile=isfile("RedbullHubConfig.json") end)
        if not hasFile then return end
        local ok,cfg=pcall(function() return HttpService:JSONDecode(readfile("RedbullHubConfig.json")) end)
